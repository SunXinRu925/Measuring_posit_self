...
data_q <- all_data_cleaned%>%
  select(matches("_al$"), -starts_with(c("phq", "gad", "SGPS", "swb","domain")))
data_q <- scale(data_q)
...
# 读取CSV文件中的第1到131列
data_z_PCA1 <- read_csv("data_z.csv")[, 1:131]
# 对这131列进行PCA降维处理
...
library(psych)
# 进行探索性因素分析
efa_result <- fa(r = cor(data_z_PCA1), nfactors = 6, rotate = "none", fm = "minres")

# 打印EFA结果
print(efa_result)

# 查看特征值
print(efa_result$values)

# 绘制碎石图
fa.plot(efa_result, main = "Scree Plot")
