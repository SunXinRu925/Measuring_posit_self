...
```{r}
# 进行探索性因素分析
efa_q <- fa(r = cor(data_q), nfactors = 6, rotate = "none", fm = "minres")
# 打印EFA结果
print(efa_q)
# 提取因子载荷
efa_q_loadings <- efa_q$loadings
# 将因子载荷保存为.csv文件
write.csv(efa_q_loadings, "efa_q_loadings.csv")
# 提取特征值
eigenvalues_q <- efa_q$values
# 绘制碎石图
plot(eigenvalues_q, type = "b", pch = 19, lwd = 2,
     xlab = "Factors", ylab = "Eigenvalues_q", main = "Scree Plot")

# 标注肘点纵坐标
# 寻找第一个特征值小于1的因子
elbow_index <- which(eigenvalues_q < 1)[1]
if(length(elbow_index) > 0) {
  # 标注肘点
  points(elbow_index, eigenvalues_q[elbow_index], col = "red", pch = 4, cex = 2)
  text(elbow_index, eigenvalues_q[elbow_index], labels = round(eigenvalues_q[elbow_index], 2), pos = 3, col = "red")
} else {
  # 如果所有特征值都大于或等于1，则没有明显的肘点
  print("No eigenvalue_q less than 1, no clear elbow point.")
}
```
...
```{r}
# 读取CSV文件
data_z_PCA2 <- read_csv("data_z.csv")[, 139:152]
# 进行PCA降维处理
...
# 进行探索性因素分析
efa_result2 <- fa(r = cor(data_z_PCA2), nfactors = 6, rotate = "none", fm = "minres")
# 打印EFA结果
print(efa_result2)
# 提取因子载荷
