...
fit <- sem(model_formula, data = data_z, std.lv = TRUE, fixed.x = F, orthogonal = TRUE)
# 提取因子得分
factor_scores <- predict(fit, type = "factor.scores")
...
head(combined_data)
set.seed(123)
rf_split <- initial_split(combined_data, prop = .7)# 70%训练集，30%测试集
...
X_train_ = rf_train[1:352 , 2:13]
y_train_ = rf_train[1:352,15]#SGPS
...
#tune the model调参
set.seed(123)
tuneRF(rf_train[1:352 , 2:13],rf_train[1:352,15],stepFactor=3)
```

```{r}
#SGPS
X_test = rf_test[1:151 , 2:13]
y_test = rf_test[1:151,15]#SGPS
...
head(result)  
ggplot(  ) + 
  geom_point(aes(x = X_test$F1, y = y_test, color  = 'red', alpha = 0.5) ) + 
  geom_point( aes(x = X_test$F1 , y = predictions, color = 'blue',  alpha = 0.5)) + 
...
```{r}
#gad
y_test_gad = rf_test[1:151,16] #gad
y_train_gad =rf_train[1:352,16] #gad
...
```{r}
#phq
y_test_phq = rf_test[1:151,17]#phq
y_train_phq =rf_train[1:352,17] #phq
...
```{r}
#SWB
y_test_SWB = rf_test[1:151,18]#SWB
y_train_SWB =rf_train[1:352,18] #SWB
...
df <- combined_data
df <- df %>%
  pivot_longer(cols = starts_with("F"), names_to = "Component", values_to = "Value")
  pivot_longer(
...
ggplot(df, aes(x = c("F1","F2", "F3","F4", "F5", "F6",'F7', "F8", "F9", 'F10', "F11", "F12"), y = IncMSE, shape = variable, color = variable)) +
