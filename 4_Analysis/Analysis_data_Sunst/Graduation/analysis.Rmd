...
#文件夹中无data.csv文件
# 删除 ID 列
cor_SEE <- all_data[, -which(names(all_data) == "ID")]
#cor_SEE<- sapply(cor_SEE, as.numeric)
cor(cor_SEE)
```
...
```{r}
#Addition:
#从all_data数据框中移除了名为"ID"的列，并将剩余的数据赋值给新的变量cor_SEE
write.csv(cor_SEE,"cor_SEE.csv")
```


```{r}
#Import Graduation文件夹中的ALT2_all.csv文件
ALT2_all<-ALT2_all%>%
  filter(ID!="phase_017_subj_13")
write.csv(ALT2_all,"ALT2_all.csv")
```
```{r}
#Import Graduation文件夹中的SRET.csv文件
SRET<-SRET%>%
  filter(ID!="phase_017_subj_13")
...
```{r}
col<-cor_SEE%>%select(ability_ALT_rt,moral_ALT_rt,ability_ALT_d,moral_ALT_d,ability_IAT,moral_IAT,ability_SRET_EW,moral_SRET_EW,ability_SRET_rt,moral_SRET_rt,ability_SRET_RJ1_d,moral_SRET_RJ1_d,ability_SRET_RJ2_d,moral_SRET_RJ2_d)
row<-cor_SEE%>%select(-c("ability_ALT_rt","moral_ALT_rt","ability_ALT_d","moral_ALT_d","ability_IAT","moral_IAT","ability_SRET_EW","moral_SRET_EW","ability_SRET_rt","moral_SRET_rt","ability_SRET_RJ1_d","moral_SRET_RJ1_d","ability_SRET_RJ2_d","moral_SRET_RJ2_d"))

#添加
cor_matrix <- cor(col)
corrplot::corrplot(cor_matrix)

#ggplot(data.frame(cor_SEE), aes(colnames(col), colnames(row), fill = cor_SEE[, 1])) +
...
```{r}
#更改路径，需要文件：
#day3_q_all.csv, SRET_all.csv, ALT1_all.csv, day0_all.csv, day1_q_all.csv, day2_q_all.csv, IAT_all.csv, ALT2_all.csv
...
#运行结果：Warning in GPFoblq(A, Tmat = Tmat, normalize = normalize, eps = eps, maxit = maxit,  :
