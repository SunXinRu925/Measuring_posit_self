...
ALT2_all <- read.csv("ALT2_all.csv", stringsAsFactors = FALSE)

# 查看数据框的前几行以确认数据已正确读取
head(ALT2_all)
is.data.frame(ALT2_all)
...
#添加
# 使用 read.csv 函数读取 CSV 文件
SRET<- read.csv("SRET.csv", stringsAsFactors = FALSE)
...
#添加
# 使用 read.csv 函数读取 CSV 文件
cor_SEE<- read.csv("cor_SEE.csv", stringsAsFactors = FALSE)
...
```{r}
col<-all_data%>%select(ability_ALT_rt,moral_ALT_rt,ability_ALT_d,moral_ALT_d,ability_IAT,moral_IAT,ability_SRET_EW,moral_SRET_EW,ability_SRET_rt,moral_SRET_rt,ability_SRET_RJ1_d,moral_SRET_RJ1_d,ability_SRET_RJ2_d,moral_SRET_RJ2_d)
row<-all_data%>%select(-c("ability_ALT_rt","moral_ALT_rt","ability_ALT_d","moral_ALT_d","ability_IAT","moral_IAT","ability_SRET_EW","moral_SRET_EW","ability_SRET_rt","moral_SRET_rt","ability_SRET_RJ1_d","moral_SRET_RJ1_d","ability_SRET_RJ2_d","moral_SRET_RJ2_d"))
...
# ggplot(data.frame(all_data), aes(colnames(col), colnames(row), fill = all_data[, 1])) +
#   #ggplot(data.frame(cor_matrix), aes(row, col, fill = cor_matrix)) +
#   geom_tile(color = "white") +
#   scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0) +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
#   labs(x = "任务", y = "量表", fill = "相关系数") +
#   coord_fixed() # 确保x和y轴标签对齐
```
...
#数据加载
```{r}
day3_q_all<-read.csv("../../Analysis_data_Sunst/Graduation/day3_q_all.csv")

SRET<-read.csv("../../Analysis_data_Sunst/Graduation/SRET_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
ALT1_all<-read.csv("../../Analysis_data_Sunst/Graduation/ALT1_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
day0_all<-read.csv("../../Analysis_data_Sunst/Graduation/day0_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
day1_q_all<-read.csv("../../Analysis_data_Sunst/Graduation/day1_q_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
day2_q_all<-read.csv("../../Analysis_data_Sunst/Graduation/day2_q_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
IAT_all<-read.csv("../../Analysis_data_Sunst/Graduation/IAT_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
ALT2_all<-read.csv("../../Analysis_data_Sunst/Graduation/ALT2_all.csv")%>%
  subset(., ID %in% day3_q_all$ID)
```
...
####SRET指标提取
```{r}
SRET_EW<-SRET_all  #上一步骤已将SRET.0重命名为SRET_all,因此本段代码将SRET.0修正为SRET_all #改动

 rt <- Filter(is.function, mget(ls(pattern = "rt"), inherits = TRUE))# 如果找到了，你可以将它们替换为正确的数值
 if(length(rt) > 0) {
  for(name in names(rt)) {
    assign(name, as.numeric(0), inherits = TRUE)  # 或者其他的默认数值
  }
} #新增，rt原数据类型为function，利用代码更改数据类型
...
all_data%>%
  ...
  scale(.)%>%
cor(.)%>%
corrplot(., order = 'original', col.lim=c(-0.5, 0.5)) %>% #在使用 corrplot函数时，指定的颜色限制（col.lim）没有覆盖相关系数矩阵中的所有值
...
#CorMat_FDR <- FDRnetwork(cor_auto(data[,3:153]),cutoff=0.1, method = "pval")#lfdr
...
aen_glasso_q <- qgraph::EBICglasso(cor(data[,3:153]), n=503, gamma = 0.5)  #,returnAllResults=TRUE，扣除ID列，共151列
...
##task的聚类
```{r}
#write.csv(structure_task_6[["loadings"]],"task_loadings.csv") structure_task_6文件夹不存在，因此需要在Files中手动import
task_loadings<-read.csv("task_loadings.csv")
task_loadings<-task_loadings%>%select(-X) #task_loadings中的X列为非数值型，不能对其进行分析，须剔除
...
```
```{r}
#此步骤需安装package_name包，若您的R版本不可用该包，请安装dendextend。
plot(color_branches(as.dendrogram(dendro), k = 3))
```
##量表的聚类
```{r}
# write.csv(structure_q_9[["loadings"]],"q_loadings.csv")structure_q_9文件夹不存在，因此需要在Files中手动import
q_loadings<-read.csv("q_loadings.csv")
q_loadings<-q_loadings%>%select(-X) #q_loadings中的X列为非数值型，不能对其进行分析，须剔除
...
###量表
```{r}
write.csv(structure_q_9[["loadings"]],"q_loadings.csv")
q_loadings<-read.csv("q_loadings.csv")
distances2 <- dist(q_loadings, method="euclidean")
 dendro2 <- hclust(distances2, method="complete")
...
```
```{r}
write.csv(factor.scores(data[3:136],f=structure_q_9)$score,"q_factor_score.csv")
...
```
```{r}
write.csv(factor.scores(data[3:151],f=structure_all_9)$score,"test_score.csv")
```
...
# object 'health_var' not found
SEE_EFA_score<-factor.scores(data[3:151],f=structure_all_9)$score%>%
...
```{r}
#factor.scores(day3_q_cor,f=day3_q_fa)
SEE_EFA_score2<-factor.scores(data[3:136],f=structure_q_9)$score%>%
  cbind(data%>%select(ID))
...
```{r}
#factor.scores(day3_q_cor,f=day3_q_fa)
SEE_EFA_score3<-factor.scores(data[137:151],f=structure_task_6)$score%>%
  cbind(data%>%select(ID))
...
#data中前两列为非数值型，需除去
# 进行探索性因子分析
efa_results <- fa(data[3:151], nfactors = 3, rotate = "varimax", scores = TRUE)
...
#复现时未找到RF_data,尝试使用RF替代RF_data。

set.seed(123)
rf_split <- initial_split(RF, prop = .7)# 70%训练集，30%测试集
...
#通过 num_columns <- ncol(rf_train)检查列数，发现rf_train只有5列，因此将代码更改为[2:5]:
X_train_ = rf_train[1:352 , 2:5]
y_train_ = rf_train[1:352,2:5]#SGPS

# 检查数据集长度
print(length(X_train_))
print(length(y_train_))

df_tmp = rf_train[, 2:5]
#Warning in randomForest.default(x = X_train_, y = y_train_, type = regression,：The response has five or fewer unique values.  Are you sure you want to do regression? 运行显示不适合回归分析且y_train与y_train长度不一致
rf.all.factor <- randomForest(SGPS ~ ., data=df_tmp, type = regression, ntree = 100,importance = TRUE)

```
```{r}
#tune the model调参
set.seed(123)
tuneRF(X_train,y_train,stepFactor=3)
```

```{r}
X_test = rf_test[1:151 , 2:5]
y_test = rf_test[1:151,2]#SGPS
...
y_test_gad = rf_test[1:151,2]#gad


y_train_gad =rf_train[1:352,2] #gad
